...
- **Modals** (`WorkLightbox`, `BookingModal`) animate through `AnimatePresence`, so they never `return null` — the conditional is the *child*. Consequence: with the modal closed the component body still renders, and in `WorkLightbox` both `w` and `clip` are `null`; anything **outside** the `AnimatePresence` child that dereferences them must be gated (`{w ? … : null}`, as the Vaul `Drawer.Root` sibling is) or every render throws. Shared props come from `useModalMotion()`; its overlay sets `pointerEvents:"none"` on exit, because the overlay now lingers in the DOM for the whole ~0.2 s exit and would otherwise keep swallowing clicks. The scroll lock goes through the shared `setScrollLock(id, on)` (one owner set for both modals — the lightbox does not trap focus, so booking can be opened on top of it) and is released in `onExitComplete`, not at the start of the exit.
