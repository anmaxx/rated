...
### 4.3. Ховеры → `whileHover` (полный инвентарь)

Полный список (сейчас — смесь CSS `:hover` и React-state+inline-transition):

| Цель | Что анимируется | Где | Примечание |
|---|---|---|---|
| Карточка работы | `scale(1.06)` + filter + scrim + подпись | `sections.jsx:316–319` | React-state, разные `.7/.4/.35s` |
| Строка услуги | фон + номер + сдвиг стрелки `translateX` | `sections.jsx:1046–1050` | React-state |
| Пункт списка «03» | фон | `index.html:104`, `sections.jsx:996` | CSS |
| Соц-иконки футера | цвет/фон | `sections.jsx:1228` | inline |
| Ссылка отзывов | цвет/сдвиг | `sections.jsx:1123` | inline |
| Точка DotNav → подпись | показ подписи (`opacity`/`translateX`) | `index.html:94` | **ховер родителя → анимация ребёнка** |
| Стрелки лайтбокса `.rt-lb-nav` | фон | `index.html:116` | CSS |
| Стрелки ленты | фон/непрозрачность | `sections.jsx:708` | — |
| Кнопки стилей | подчёркивание/цвет | инлайн | — |
| Глобально `a` | `transition: color` | `index.html:79` | оставить на CSS |
| Шапка при скролле («затвердевание») | фон/бордер по `scrollY` | `sections.jsx:165–167` | **скролл-триггер, не ховер** → кандидат в `useScroll`, либо оставить JS |

**Оговорки (заложить, иначе регресс):**
- `whileHover={{ backgroundColor: "var(--accent)" }}` — Motion твинит между конкретными цветами; тван «к `var()`» может не сгладиться как CSS `transition`. → резолвить var()→конкретное значение, либо оставить эти ховеры на CSS.
- Паттерн «ховер родителя → анимация ребёнка» (DotNav) требует **variant-propagation**, а не однострочника. → variants или оставить на CSS.
- `:focus-visible` — не трогаем (доступность, не анимация).

### 4.4. Фейд ролика «03» → `motion` opacity

`sections.jsx:946`: `opacity: fade?0:1, transition:"opacity .26s ease"` → `motion.div` `animate={{opacity: fade?0:1}}` `transition={{duration:.26}}`. Поведение 1:1.
...
6. **Откат.** Пуш в master = сразу прод (`mem:rated-tattoo-live-prod`). Для каждой фазы: откат = `git revert` merge-коммита PR → авто-redeploy. Для Фазы 3 (лента) — **gate «владелец подтвердил `vite preview`» до мержа**.
...
**Общее:** консоль без ошибок ПО НАШЕМУ КОДУ — базлайн шума: Vaul/Radix (aria-hidden), Яндекс-плеер (CORS/deprecated) — их отделять. Мобильный прогон (реальный iOS Safari/Android Chrome) для Фаз 1 и 3.
